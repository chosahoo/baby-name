...
# -*- coding: utf-8 -*-
"""
대법원 인명용 한자 데이터를 파싱하여 JavaScript 형식으로 변환하는 스크립트 (v2)
획수 정보는 cjklib 라이브러리 사용
"""
...
try:
    ...
    cjk = characterlookup.CharacterLookup('T')  # Traditional Chinese
    use_cjklib = True
    print("cjklib 사용 가능")
except Exception as e:
    print(f"cjklib 초기화 실패: {e}")
    print("기본 획수 추정 사용")
    use_cjklib = False
...
def get_stroke_count(char):
    """한자 획수 계산
    cjklib를 사용하여 정확한 획수 반환
    """
    if use_cjklib:
        try:
            # kTotalStrokes 시도
            strokes = cjk.getStrokeCount(char)
            if strokes:
                return strokes
        except:
            pass

    # cjklib 사용 불가 시 유니코드 기반 추정
    # Unihan 데이터베이스의 일반적인 획수 범위
    code_point = ord(char)
    ...
    if 0x4E00 <= code_point <= 0x62FF:  # CJK Unified Ideographs
        # 범위 내에서 선형 분포 가정
        offset = code_point - 0x4E00
        # 약 20,000자를 1~30획에 분포
        return min(30, max(1, offset // 600 + 1))
    elif 0x3400 <= code_point <= 0x4DBF:  # CJK Extension A
        offset = code_point - 0x3400
        return min(30, max(1, offset // 200 + 1))
...
def get_element(char, meaning):
    ...
    """
    # 의미 기반 간단한 분류
    wood_keywords = ['나무', '목', '숲', '초', '풀', '꽃', '죽', '竹', '木', '松', '梅', '林', '柳', '桂', '枝']
    fire_keywords = ['불', '화', '밝', '빛', '햇', '볕', '炎', '火', '日', '明', '光', '陽', '照', '耀', '煥']
    earth_keywords = ['흙', '토', '땅', '산', '석', '石', '土', '山', '田', '地', '壤', '坤']
    metal_keywords = ['쇠', '금', '은', '철', '金', '銀', '鐵', '錢', '利', '鋼']
    water_keywords = ['물', '수', '강', '바다', '비', '눈', '水', '江', '海', '河', '雨', '雪', '淸', '澤', '泉', '淵']

    for keyword in wood_keywords:
        if keyword in meaning or keyword in char:
            return '목(木)'
...
def main():
    ...
    output_lines.append("// - 획수는 Unihan 데이터베이스 기준")
...
""")
    ...
    # 파일 저장
    with open('hanjaData-full-v2.js', 'w', encoding='utf-8') as f:
        f.write('\n'.join(output_lines))

    print(f"\n완료! hanjaData-full-v2.js 파일이 생성되었습니다.")
    print(f"총 {len(sorted_readings)}개의 음절에 대한 한자 데이터가 포함되었습니다.")

    # 샘플 몇 개 확인
    print("\n샘플 데이터 확인:")
    for reading in ['가', '나', '다'][:3]:
        if reading in hanja_by_reading:
            print(f"\n'{reading}': {len(hanja_by_reading[reading])}개 한자")
            for hanja in hanja_by_reading[reading][:3]:
